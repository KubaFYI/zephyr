...
#if IS_ENABLED(CONFIG_SDI_12_TX_ENABLE_INV)
...
/**
 * @brief Sends an extended command to a sensor and outputs its response via a
 *        parameter
 *
 * @param uart_dev Device handle for the UART interface used as as the data line
 * @param address Address of the sensor used
 * @param ext_cmd[in] Pointer to a null-terminated string of an extended command
 * @param ret_str[out] Pointer to an buffer for the command response to be
 *        placed (if any). Must be large enough to fit the return value as
 *        defined by the sdi_12 device manufacturer.
 *
